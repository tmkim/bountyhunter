...
export default function DetailsPanel({ card, deck, deckPrice, costData, rarityData, cardPriceHistoryData, isLoading }: Props) {
    const getLeftMargin = (data: HistoryData[]) => {
      if (!data || data.length === 0) return 30;
      const maxPrice = Math.max(...data.map((d) => d.price));
      // Estimate number of digits to set margin width
      const digitCount = maxPrice.toFixed(2).length;
      return (digitCount * 3) - 10; // 20px base, scaled by digit count, capped at 60
    };
    return (
                    ...
                    <div className="flex-1 border rounded p-2 mt-2">
                        ...
                        {card ? (
                          ...
                          <div className="bg-gradient-to-b from-gray-50 to-white border border-gray-200 rounded-xl p-4 shadow-inner h-[320px]">
                            {isLoading ? (
                              ...
                              <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={cardPriceHistoryData}
                                          margin={{ top: 10, right: 0, left: getLeftMargin(cardPriceHistoryData), bottom: 0 }}>
                            ...
                            <span className="text-2xl text-black">Right-Click card to preview</span>
                        )
                        }
                    </div>
